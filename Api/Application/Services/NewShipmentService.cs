...
{
    public class NewShipmentService : IShipmentService
    {
        ...
        public NewShipmentService(IShipmentRepository shipmentRepository, IShipmentResourceRepository shipmentResourceRepository, IBalanceService balanceService, IUnitRepository unitRepository, IResourceRepository resourceRepository, IClientRepository clientRepository) 
        { 
        ...
        public async Task<Shipment> UpdateShipmentAsync(CreateShipmentDto dto)
        {
            ...
            {
                var resource = new ShipmentResource { Id = Guid.NewGuid(), ShipmentId = oldShipment.Id, ResourceId = item.ResourceId, UnitId = item.UnitId, Quantity = item.Quantity };
        ...
        public async Task<Shipment> DeleteShipmentAsync(Guid id)
        {
            ...
            if (shipment == null) 
                throw new NotFoundException($"Документ отгрузки с ID {id} не найден");

            if (shipment.IsSigned)
                throw new ConflictException($"Невозможно изменить документ отгрузки с номером {shipment.Number}. Так как он подписан");
        ...
        private async Task ShipmentValidateAsync(CreateShipmentDto newShipment, IEnumerable<ShipmentResource> oldShipmentResources)
        {
            ...
            if(oldShipment.IsSigned)
                throw new ConflictException($"Невозможно изменить документ отгрузки с номером {oldShipment.Number}. Так как он подписан");
